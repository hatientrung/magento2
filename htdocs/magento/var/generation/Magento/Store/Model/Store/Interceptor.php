 ...
 */
class Interceptor extends \Magento\Store\Model\Store
{
    /**
    ...
    public function __construct(\Magento\Framework\Model\Context $context, \Magento\Framework\Registry $registry, \Magento\Store\Model\Resource\Store $resource, \Magento\MediaStorage\Helper\File\Storage\Database $coreFileStorageDatabase, \Magento\Framework\App\Cache\Type\Config $configCacheType, \Magento\Framework\UrlInterface $url, \Magento\Framework\App\RequestInterface $request, \Magento\Config\Model\Resource\Config\Data $configDataResource, \Magento\Framework\Filesystem $filesystem, \Magento\Framework\App\Config\ReinitableConfigInterface $config, \Magento\Store\Model\StoreManagerInterface $storeManager, \Magento\Framework\Session\SidResolverInterface $sidResolver, \Magento\Framework\Stdlib\Cookie\CookieMetadataFactory $cookieMetadataFactory, \Magento\Framework\Stdlib\CookieManagerInterface $cookieManager, \Magento\Framework\App\Http\Context $httpContext, \Magento\Framework\Session\SessionManagerInterface $session, \Magento\Directory\Model\CurrencyFactory $currencyFactory, $currencyInstalled, \Magento\Framework\Data\Collection\Db $resourceCollection = null, $isCustomEntryPoint = false, array $data = array())
    {
        $this->___init();
        parent::__construct($context, $registry, $resource, $coreFileStorageDatabase, $configCacheType, $url, $request, $configDataResource, $filesystem, $config, $storeManager, $sidResolver, $cookieMetadataFactory, $cookieManager, $httpContext, $session, $currencyFactory, $currencyInstalled, $resourceCollection, $isCustomEntryPoint, $data);
    }
    ...
    {
        $pluginInfo = $this->pluginList->getNext($this->subjectType, 'setCookie');
        if (!$pluginInfo) {
            return parent::setCookie();
        } else {
            return $this->___callPlugins('setCookie', func_get_args(), $pluginInfo);
        }
    }
    ...
    public function deleteCookie()
    {
        ...
        if (!$pluginInfo) {
            return parent::deleteCookie();
        } else {
            return $this->___callPlugins('deleteCookie', func_get_args(), $pluginInfo);
        }
    }
